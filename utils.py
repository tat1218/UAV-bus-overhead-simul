...
def simulation(simul_time, uavs:list[UAV.UAV], buses:list[Bus.Bus], scheme="Game", budget=BUDGET, random_task_range={'min':1,'max':1}):
    # 시뮬레이션 메인 함수
    ...
    for simul_i in range(simul_time):
        random_value = random.randint(random_task_range['min'], random_task_range['max'])
        task_cpu_cycle = random_value
        task_data_size = random_value
        task_delay = random_value
		...
		# UAV, 버스 초기화 & 이동
        for uav in uavs:
            uav.init(task_cpu_cycle,task_data_size,task_delay,budget)
        ...
        if random_price:
            for bus in buses:
                bus.change_price(0.5+random.random()*1.5)
        else:
            while(iteration):
                iteration = 0
                iter_count +=1
