...
class UAV:
    ...
    def purchase_cpu(self, i, bus_id_list, buses,sum,num, T_tran, T_off, E_tran, E_off):
        cost = 0
                ...
                if bus.id == bus_id_list:
                    ...
                    if max_cpu > 0:
                        ...
                        if cost > 0 and temp_t <= self.task[2] :
                            ...
                            self.purchase_bus_list.append([bus.id, max_cpu, round(max_cpu/bus_cpu_cycle,2)])
        ...
        while(iteration):
            ...
            for bus in buses:
                ...
                if bus.uav_list:
                    ...
                    #print ("AAA", [bus.id, temp_price, bus.price, demand_sum_from_uav, bus.MAX_CPU, (demand_sum_from_uav - bus.MAX_CPU), round((demand_sum_from_uav - bus.MAX_CPU) / sigma_speed, 14)])
                    if abs(temp_price - bus.price) > 1 / 10e14:
            ...
            for uav in uavs:
                ...
                if uav.bus_list:
                    ...
                    for i in range(len(bus_id_list)):
                        ...
                        cost = uav.purchase_cpu(i, bus_id_list[i], buses,summa,price_num, T_transmission, T_offload, E_transmission, E_offload)
