...
if __name__ == "__main__":
    ...
    for i in range(NUM_UAV_STEP):
        ...
        for j in range(NUM_BUS_STEP):
            ...
            # simulate
            simulation(SIMUL_TIME,uavs,buses)
            ...
            uav_bus_avg_overhead[i][j] = round(Average(tmp_overhead), 2)
            uav_avg_utility[i][j] = round(Average(tmp_uav_utility), 2)
            bus_avg_utility[i][j] = round(Average(tmp_bus_utility), 2)
